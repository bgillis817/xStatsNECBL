  ...
  all_pa <- raw_data %>%
    ...
    mutate(
      pa_outcome = case_when(
        ...
        PlayResult == "Walk"                                    ~ "walk",
        PlayResult == "HitByPitch"                              ~ "hit_by_pitch",
        PlayResult == "Strikeout"                               ~ "strikeout",
        KorBB == "Walk"                                         ~ "walk",
...
# calculate_expected_xwoba_and_full: called by initialize_enhanced_data_composite
# in the Shiny server when a session starts.
